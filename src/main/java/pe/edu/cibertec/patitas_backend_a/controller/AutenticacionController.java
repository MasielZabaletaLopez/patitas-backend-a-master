...
public class AutenticacionController {
    ...
    @PostMapping("/logout")
    public SignOutResponseDTO logout(@RequestBody SignOutRequestDTO logoutRequestDTO) {
        try {

            autenticacionService.CierreSesion(logoutRequestDTO);

            return new SignOutResponseDTO("01", "Cierre Sesion exitoso");
        } catch (IOException e) {
            return new SignOutResponseDTO("98", "Error : " + e.getMessage());
        }
    }
}